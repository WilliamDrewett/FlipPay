...
library TimelocksLib {
    ...
    function set(
        ...
        uint256 deployedAt = Timelocks.unwrap(timelocks) >> _DEPLOYED_AT_OFFSET;
        require(timestamp >= deployedAt, "Timelocks: timestamp before deployment");

        uint32 relativeOffset = uint32(timestamp - deployedAt);
        uint256 shift = uint256(stage) * 32;
        uint256 mask = ~(uint256(0xffffffff) << shift);

        uint256 data = (Timelocks.unwrap(timelocks) & mask) | (uint256(relativeOffset) << shift);
